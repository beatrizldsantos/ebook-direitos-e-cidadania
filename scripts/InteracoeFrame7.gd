...
@export var area_limite: Control 
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch:
		...
		if not event.pressed and event.button_index == MOUSE_BUTTON_LEFT and touch_index == -1:
			_on_drag_end()
			
	elif event is InputEventScreenDrag:
		if event.index == touch_index:
			var global_pos = get_canvas_transform().affine_inverse() * event.position
			_atualizar_posicao_arraste(global_pos)
...
func _on_drag_button_input(event: InputEvent) -> void:
	...
	if event is InputEventScreenTouch and event.pressed:
		...
		_iniciar_arraste(drag_button.get_global_transform() * event.position)
	elif event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		touch_index = -1
		_iniciar_arraste(drag_button.get_global_transform() * event.position)

func _iniciar_arraste(global_pos: Vector2) -> void:
	is_dragging = true
	drag_offset = global_pos - drag_button.global_position
	drag_button.move_to_front()

func _atualizar_posicao_arraste(mouse_pos: Vector2) -> void:
